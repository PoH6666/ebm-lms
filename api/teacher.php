...
function getDB() {
    ...
    if ($conn->connect_error) {
        respond(['status' => 'error', 'message' => 'DB failed: ' . $conn->connect_error]);
    }
...
if ($action === 'ping') {
    respond(['status' => 'ok', 'message' => 'teacher.php alive', 'php' => PHP_VERSION]);
}

// Get teacher_id
$teacher_id = (int)($_POST['teacher_id'] ?? $_GET['teacher_id'] ?? $_SESSION['user_id'] ?? 0);
if (!$teacher_id) {
    respond(['status' => 'error', 'message' => 'Not authenticated. Send teacher_id.']);
}
...
switch ($action) {
    ...
    case 'get_my_subjects':
        $stmt = $conn->prepare("SELECT id, subject_name, subject_code, description, status, created_at FROM subjects WHERE teacher_id = ? ORDER BY created_at DESC");
    ...
    case 'create_subject':
        ...
        $chk = $conn->prepare("SELECT id FROM subjects WHERE subject_code = ?");
        ...
        if ($stmt->execute()) {
            ...
            $stmt->close();
            respond(['status' => 'success', 'message' => 'Subject created!', 'subject_id' => $new_id]);
        } else {
    ...
    case 'get_lessons':
        ...
        if ($subject_id) {
            ...
            $stmt = $conn->prepare("SELECT l.*, s.subject_name FROM lessons l LEFT JOIN subjects s ON l.subject_id = s.id WHERE l.teacher_id = ? ORDER BY l.created_at DESC");
            $stmt->bind_param('i', $teacher_id);
        }
        $stmt->execute();
        $result = $stmt->get_result();
    ...
    case 'get_quizzes':
        $stmt = $conn->prepare("SELECT q.*, s.subject_name FROM quizzes q LEFT JOIN subjects s ON q.subject_id = s.id WHERE q.teacher_id = ? ORDER BY q.created_at DESC");
        $stmt->bind_param('i', $teacher_id);
        $stmt->execute();
        $result = $stmt->get_result();
        $quizzes = [];
    ...
    case 'get_exams':
        $stmt = $conn->prepare("SELECT e.*, s.subject_name FROM exams e LEFT JOIN subjects s ON e.subject_id = s.id WHERE e.teacher_id = ? ORDER BY e.created_at DESC");
        $stmt->bind_param('i', $teacher_id);
        $stmt->execute();
        $result = $stmt->get_result();
        $exams = [];
